...
class CSVLogger:
    ...
    def log(self, values):
        ...
        line = ",".join(f"{v:.4f}" if isinstance(v, float) else str(v) for v in values)
...
def cutmix_data(x, y, alpha=1.0):
    ...
    lam_adjusted = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (x.size(2) * x.size(3)))
...
def train_one_epoch(
    ...
    for imgs, labels in pbar:
        ...
        correct += (
            lam * (preds == y1).sum().item() + (1 - lam) * (preds == y2).sum().item()
        )
        total += labels.size(0)
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        csv_logger.log([epoch, tr_loss, tr_acc, tr_top3, val_loss, val_acc, val_top3])
